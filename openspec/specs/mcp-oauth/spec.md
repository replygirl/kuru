...
## Purpose
Define bounded OAuth authorization for HTTP MCP aliases, including discovery, client registration, browser and device grants, refresh, revocation, and alias-local status. Keep each grant bound to its reviewed issuer, resource, client, and scopes while preventing credential or route leakage across aliases; static-header and STDIO MCP paths remain separate.
