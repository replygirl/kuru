...
Observed on macOS: `mixed_alias_failures_preserve_live_and_stale_catalogs_independently` proves live, stale, degraded and disabled aliases coexist without route/effect leakage. `oauth_status_tracks_mixed_alias_state_without_projecting_secrets` passed 1/1 in 0.72s and drives the same catalog-backed status path through authorized, disabled, refresh-required, login-required and native-store-unavailable states; serialized status and `/tools` catalog projections omit recognizable client, access and refresh secrets. Full CLI parity remains open in 5.3.
...
Observed on macOS: the typed command parser and CLI authority projection each passed 1/1; `oauth_commands_refuse_unknown_disabled_and_stdio_aliases_before_activation` passed 1/1 in 0.38s with zero HTTP requests and zero STDIO conversations. The completed-frame PTY status/quit fixture passed 1/1 in 2.91s after adding a recognizable environment-owned client secret and proves the raw terminal stream omits it. Fake-server login/logout parity remains pending before this section closes.
