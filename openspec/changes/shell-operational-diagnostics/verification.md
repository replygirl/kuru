...
- [x] 1.4 @eval (agent) inspect the public ToolHost failure string across timeout, capture, ownership, and cleanup fixtures -> focused Unix suite (17 passed) confirms timeout keeps its primary category while the fixed unconfirmed-ownership suffix is retained; capture and cleanup fixtures expose no raw chain.
