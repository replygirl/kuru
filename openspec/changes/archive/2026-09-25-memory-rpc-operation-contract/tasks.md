...
- [x] 2.1 Add `OperationContract`, `Mutation`, `Receipt` and `ReplyBudget` with
  wildcard-free `contract()` on `ServiceCall`, `ViewOperation` and
  `LedgerOperation`; verify the table test still passes.
- [x] 2.2 Rebuild `may_mutate`, `unit_receipt_method` and `receipt_progress_key`
...
- [x] 2.3 Add `AttachmentState::holds_handles()`/`holds_resources()` at the
...
- [x] 3.1 Add the wire-surface fixture and pin test with serde-derived coverage
  and a refusing bless path; verify it passes at 1.7 and fails on a temporary
  wire edit without a bump.
- [x] 3.2 Document the protocol-change procedure in `docs/development.md`.

## 4. No-behavior-change verification

- [x] 4.1 Run the full `kuru-memory` package test task, clippy `-D warnings`
