...
Post-archive delivery correction, 2026-09-22: the exact P02a pre-push coverage found one PTY expectation mismatch: the test waited for lowercase `unknown command` while the completed UI frame rendered `Unknown command; use /help`. The assertion now waits for that exact rendered message and composer-ready frame. The focused real-PTY case passed 1/1 under the authorized local loopback profile; the initial restricted-sandbox attempt stopped at Dolt's loopback bind before reaching the command assertion. The preceding failed coverage run had no other failed target. Replacement full coverage and hosted native CI remain delivery gates for the amended head.
