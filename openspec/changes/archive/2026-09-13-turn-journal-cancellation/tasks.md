...
- [x] 4.3 Run the focused memory, connector, runtime, TUI, format, lint, typecheck, and docs checks, then the single coordinated workspace coverage task; record exact exits and retain the 90 percent floor
- [x] 4.4 Run native macOS/Linux and Windows real-Dolt, process-owner, A2A, and PTY cancellation fixtures; record platform-specific evidence and defer only genuinely unavailable native runs
