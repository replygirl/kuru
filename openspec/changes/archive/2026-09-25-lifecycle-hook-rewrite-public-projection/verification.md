...
- **Flake.** `parallel_post_hooks_settle_independently_but_rejoin_in_original_call_order` failed once at `hook_tests.rs:1146`, the settle-order assertion, under the concurrent group run. It passed three isolated reruns and the full-group rerun. The test involves no pre-turn rewrite or public projection, so the failure is not attributed to this change.
