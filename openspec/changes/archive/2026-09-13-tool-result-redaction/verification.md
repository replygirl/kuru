...
- [x] 4.3 @e2e (agent) run 4.1 and 4.2 on native Windows with the supported PowerShell/process and real-memory fixtures -> PR14 CI run 34747712455 passed the complete Windows instrumented suite, including direct CLI MCP projection, `tool_projection_redacts_results_without_altering_files_or_arguments`, `isolated_toolhost_windows_shell_receives_projected_environment`, `native_model_tool_replay_preserves_authority_and_returns_real_receipts`, and the runtime marker-limit cases; `/private/tmp/kuru-pr14-windows-ci-86f.log`.
