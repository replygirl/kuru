...
The global `--debug` flag SHALL add only documented operational detail to the application's fixed target allowlist. It MUST NOT enable `RUST_LOG`, arbitrary third-party logs, raw payloads, or unsolicited stdout/stderr/TUI output.

#### Scenario: Machine output
- **WHEN** `kuru run --json` runs with either normal or debug diagnostics
- **THEN** stdout remains the exact parseable command result and diagnostic files contain no fake secret sentinel.
