...
- [ ] 1.2 @runtime (agent) run all four package shards and aggregate their raw profiles at the exact pull-request head on CI Windows 2025 -> Cargo invokes every declared standard test target in each shard, the ledgers select every target exactly once across the package partition, and the full Windows workspace remains at or above 90% line coverage
