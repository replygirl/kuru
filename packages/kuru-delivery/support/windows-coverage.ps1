...
# Every shard failure after this point, including verifier, toolchain and
# inventory setup, leaves its error, manifests and runner ledger beside the
# per-test output logs and stall reports for the failure diagnostics upload.
try {
    ...
    if ($null -ne $diagnosticsDir) {
        Set-Content -LiteralPath (Join-Path $diagnosticsDir 'failure.txt') -Encoding utf8NoBOM -Value ($_ | Out-String)
        if ($null -ne $state) {
