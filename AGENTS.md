...
`openspec/schemas`, `.claude`, `.agents`, `.codex` or `.opencode` generated files. Regenerate those with
`mise run cospec -- update`; check drift with `cospec:managed:check`.

## Documentation and security

AGENTS.md is the canonical repository instruction source. CLAUDE.md imports it
using `@AGENTS.md`. Update these instructions when repository workflow changes,
